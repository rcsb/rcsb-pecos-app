    ...
    for (let k = 0; k < results.length; k++) {
        ...
        for (let i = 0; i < alignment.structures.length; i++) {
            if (k === 0 && i === 0 || i > 0) {
                ...
                const dat = filtered.length > 0 ? filtered[0] : null;
                info.push({
                    entry_id: name,
                    asym_id: sele.asym_id,
                    auth_asym_id: dat?.auth_asym_id,
                    pdbx_description: dat?.pdbx_description,
                    rcsb_sample_sequence_length: dat?.rcsb_sample_sequence_length || alignment.sequence_alignment![i].sequence?.length,
