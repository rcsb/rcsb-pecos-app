...
export default function PlusSignIcon({ disabled }) {
    const cId = (disabled) ? 'add-new-item-disabled' : 'add-new-item';
    const cFill = (disabled) ? '#808080a3' : 'currentColor';
    const cName = (disabled) ? 'icon-container plus-sign-icon-disabled' : 'icon-container plus-sign-icon';
    return (
        <span role="img" aria-label="plus-sign" className={cName}>
            <svg id={cId} viewBox="0 0 22 22" width="1em" height="1em" fill={cFill}>
